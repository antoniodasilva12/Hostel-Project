...
const LaundryRequests = () => {
  ...
  const testDatabaseConnection = async () => {
    try {
      ...
      console.log('Successfully accessed profiles table:', profileData);

      // Test if we can access the laundry_requests table
      ...
      if (laundryError) {
        console.error('Error accessing laundry_requests table:', laundryError);
        return;
      }
      
      console.log('Successfully accessed laundry_requests table:', laundryData);
    } catch (err) {
      console.error('Error in testDatabaseConnection:', err);
    }
  };
  ...
  const fetchLaundryRequests = async () => {
    try {
      ...
      console.log('Admin access verified, fetching laundry requests...');
      ...
      console.log('Fetched laundry requests:', laundryData);

      if (!laundryData || laundryData.length === 0) {
        console.log('No laundry requests found');
        setRequests([]);
        return;
      }
      ...
      console.log('Fetched student profiles:', profilesData);
      ...
      console.error('Error in fetchLaundryRequests:', err);
